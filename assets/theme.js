      ...
      value: function _updateProductPrices(newVariant, previousVariant) {
        ...
        if (!newVariant) {
          ...
          if (newVariant['compare_at_price'] > newVariant['price']) {
            ...
            productMonthlyPrices.innerHTML += "Oder zahlen Sie <span class=\" Price Text--subdued \" data-money-convertible>".concat(Currency.formatMoney(newVariant['price']/12, window.theme.moneyFormat), "</span>/montl");
            console.log(Currency.formatMoney(newVariant['price']));
            productMetaPrices.innerHTML += "<span style='color: #ba2e2e' class=\"ProductMeta__Price Price Price--compareAt Text--subdued u-h4\" data-money-convertible>".concat(Currency.formatMoney(newVariant['compare_at_price'], window.theme.moneyFormat), "</span>");
          } else {
...
(function () {
  setTimeout(function () {
    const $summary = $('.jdgm-rev-widg__summary-text');
    const summaryHtml = $summary.html();
    if (summaryHtml) {
		const splitSummary = summaryHtml.split('|');
    	$summary.html(`<b style="font-size:25px">${splitSummary[0]}</b><br><br> |${splitSummary[1]}`);
    }
  }, 1000);
});